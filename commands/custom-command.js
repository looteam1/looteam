...
client.on('interactionCreate', async interaction => {
  if (interaction.isSelectMenu() && interaction.customId === "custom-command-menu") {
    ...
    if (selectedValue === "add_custom_command") {
      ...
      const commands = Object.entries(db.all()).filter(([key, value]) => key.startsWith("custom_command_"));
      const commandList = commands.map(([key, entry]) => `**${key.replace("custom_command_", "")}**: ${entry}`).join("\n") || "Henüz eklenmiş özel komut yok.";
    ...
    if (interaction.customId === "addCustomCommandModal") {
      ...
      db.set(`custom_command_${commandName}`, commandText);
      ...
      db.delete(`custom_command_${commandName}`);
...
client.on('messageCreate', async message => {
  ...
  const commandText = db.fetch(`custom_command_${commandName}`);
